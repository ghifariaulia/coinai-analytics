...
export async function GET(request: NextRequest) {
  try {
    ...
    if (startTime) {
      ...
      const startTimeSeconds = startTimeNum > 9999999999 ? Math.floor(startTimeNum / 1000) : startTimeNum;
      params.append('startTime', startTimeSeconds.toString());
    }
    if (endTime) {
      const endTimeNum = parseInt(endTime);
      const endTimeSeconds = endTimeNum > 9999999999 ? Math.floor(endTimeNum / 1000) : endTimeNum;
    ...
    const response = await fetch(`https://api.bitget.com/api/v2/spot/market/candles?${params}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('Request URL:', `https://api.bitget.com/api/v2/spot/market/candles?${params}`);
